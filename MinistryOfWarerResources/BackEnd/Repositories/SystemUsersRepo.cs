...
namespace MinistryOfWarerResources.BackEnd.Repositories
{
    public class SystemUsersRepo : ISystemUsers
    {
        ...
        public async Task<bool> EditAllRoleToAdmin(string UserId)
        {
            ...
            else if (UserRole.RoleId == "DawaView")
            {
                await _userManager.RemoveFromRoleAsync(User, "DawaView");
                var RoleRecored = await _db.Roles.FirstOrDefaultAsync(a => a.Name == "DawaView");
                await _userManager.AddToRoleAsync(User, "admin");
                UserRole.RoleId = RoleRecored.Id;
                _db.UserRoles.Update(UserRole);
                await _db.SaveChangesAsync();
